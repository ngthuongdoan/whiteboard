...
export const ColorPickerSelection = memo(({ className, ...props }: ColorPickerSelectionProps) => {
  ...
  return (
    <div
      className={cn("relative size-full rounded cursor-crosshair", className)}
      onPointerDown={e => {
        e.preventDefault()
        setIsDragging(true)
  ...
  return (
    <div className="relative">
      <Input
        ...
        {...props}
        className={cn(
          "h-9 w-13 rounded-l-none bg-secondary px-2 text-xs shadow-none",
          className,
        )}
      />
...
export const ColorPreview = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => {
  const { hue, saturation, lightness, alpha } = useColorPicker()
  const color = Color.hsl(hue, saturation, lightness).alpha(alpha / 100)
  return (
    <div
      className={cn("size-9 rounded-md border border-slate-200 shadow-inner min-w-9", className)}
